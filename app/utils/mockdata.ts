export const processes = ['Compras', 'Ventas', 'Seguridad', 'RRHH', 'Operaciones', 'Calidad'];
...
/**
 * 
 * @param count Generador de datos aleatorios.
 * @returns 
 */
export const generateMockAudits = (count = 60) => {
  // Definimos los procesos y responsables según el dataset requerido
  ...
  return Array.from({ length: count }, (_, i) => {
    const status = getRandomElement(statuses) as 'DRAFT' | 'IN_PROGRESS' | 'DONE' | 'BLOCKED';

    // Aplicamos la regla de coherencia recomendada
    ...
    const process = getRandomElement(processes);
    ...
    return {
      ...
      progress: progress,
      owner: {
        id: `u_${(i % processes.length) + 1}`,
        name: auditors[process.toLowerCase()] || 'Alfonso'
      },
      targetDate: new Date(Date.now() + Math.random() * (new Date(new Date().getFullYear(), 11, 31).getTime() - Date.now())).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' }),
      updatedAt: new Date().toISOString(),
      createdAt: "2026-02-01T09:00:00Z",
      templateId: mockTemplates.filter((item: any) => item.process == process).reduce((acc, item: any) => item.id, '')
    };
  });
};
